<?php
     ...
     * @throws ErroreInternoDatabaseException se si verifica un errore interno al database
     */
    function getPrestitiBySede(string $id_sede): array {
        $conn = pg_connect(CONNECTION_STRING);
        if (!$conn)
            throw new ErroreInternoDatabaseException();

        $query = "SELECT * FROM biblioteca.getPrestitiBySede($1)";
        if (!pg_prepare($conn, "get_prestiti_by_sede", $query))
            throw new ErroreInternoDatabaseException();

        $result = pg_execute($conn, "get_prestiti_by_sede", array($id_sede));
        if (!$result)
            throw new ErroreInternoDatabaseException();
